...
import { Activity, AlertTriangle, ArrowLeft, Battery, Footprints, HeartPulse, MapPin, Radio, ShieldCheck, Siren, UserRoundCheck, Wifi } from "lucide-react";
    ...
    <div className="detail-layout">
      <section className="detail-main">
        <div className="detail-status-grid">
          {[["Current Status",band.status,ShieldCheck],["Venue Position",`Zone ${band.zone} / Segment ${band.segment}`,MapPin],["Heart Rate",`${band.hr} bpm`,HeartPulse],["SpO2",`${band.spo2}%`,UserRoundCheck],["Battery",`${band.battery}%`,Battery],["Connectivity",band.connectivity,Wifi]].map(([label,value,Icon]) => <article key={String(label)}><Icon size={22} /><span>{String(label)}</span><strong>{String(value)}</strong></article>)}
        </div>
        <section className="band-live-state"><header><div><i className={band.connectivity.toLowerCase()} /><span>Live wearer state</span><strong>{band.status}</strong></div><time>Updated {band.lastSeen.slice(11,19)}</time></header><div><article><Footprints size={22} /><span>Motion</span><strong>{band.motionState}</strong><small>{band.immobile ? `${band.persistenceMinutes} min without movement` : "Movement observed"}</small></article><article><AlertTriangle size={22} /><span>Fall detection</span><strong>{band.fallDetected ? "DETECTED" : "CLEAR"}</strong><small>{band.fallDetected ? "Verification required" : "No fall signature"}</small></article><article><Siren size={22} /><span>SOS input</span><strong>{band.sos ? "ACTIVE" : "INACTIVE"}</strong><small>{band.sos ? "Manual wearer signal" : "No wearer request"}</small></article><article><Radio size={22} /><span>Signal quality</span><strong>{Math.round(band.signalQuality * 100)}%</strong><small>{band.connectivity} gateway link</small></article></div></section>
        ...
        <section className="movement-timeline"><header><div><p className="eyebrow">Movement & Safety Timeline</p><h2>Recent state transitions</h2></div><Activity size={20} /></header><div>{[
          ["20:34:18", band.motionState, `Current motion classified as ${band.motionState.toLowerCase()}.`, band.status === "DISTRESSED" || band.status === "SOS" ? "danger" : "current"],
          ["20:33:51", band.fallDetected ? "FALL SIGNATURE" : "MOVEMENT SAMPLE", band.fallDetected ? "Accelerometer pattern crossed the configured fall threshold." : "Motion vector remained inside normal venue movement range.", band.fallDetected ? "danger" : "normal"],
          ["20:32:44", "ZONE TRANSITION", `Band entered Zone ${band.zone}, Segment ${band.segment} through the nearest concourse boundary.`, "normal"],
          ["20:31:29", "GATEWAY HANDOFF", `${band.connectivity} contact confirmed with ${Math.round(band.signalQuality * 100)}% signal quality.`, "normal"],
        ].map(([time,state,detail,tone]) => <article key={time} className={tone}><time>{time}</time><i /><div><strong>{state}</strong><span>{detail}</span></div></article>)}</div></section>
      ...
      <aside className="detail-rail">
        <section className="response-card"><header><Siren size={24} /><div><p className="eyebrow">Suggested Response</p><h2>{actionLabels[band.risk.recommendedAction]}</h2></div></header><p>{band.status === "NORMAL" ? "Continue passive monitoring and retain the current telemetry cadence." : "Confirm the wearer state using nearby field staff and preserve the alert audit trail."}</p><Link href={`/interventions?band=${band.id}`}>{actionLabels[band.risk.recommendedAction]}</Link><small>Operator authorization required</small></section>
