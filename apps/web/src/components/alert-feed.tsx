...
export function AlertFeed() {
  ...
  return <main className="ops-module-page">
    ...
    <section className="module-title-band"><div><p className="eyebrow">Incident Operations</p><h1>Live Alerts & Incidents</h1><p>Explainable Human, Crowd and Population Integrity alerts with operator-controlled lifecycle.</p></div><div className="module-title-status"><i /><span>Live feed</span><strong>12 event alerts</strong></div></section>
    ...
    <div className="alert-workspace">
      <section className="alert-feed-list">
        <header><div className="module-filter-search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search alerts, zones, bands" /></div><select value={category} onChange={(event) => setCategory(event.target.value as typeof category)}><option>ALL</option><option>HUMAN_RISK</option><option>CROWD_RISK</option><option>POPULATION_INTEGRITY</option></select><select value={severity} onChange={(event) => setSeverity(event.target.value as typeof severity)}><option>ALL</option><option>critical</option><option>high</option><option>moderate</option><option>low</option></select><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option>ALL</option><option>NEW</option><option>ACKNOWLEDGED</option><option>RESOLVED</option></select><button title="Clear alert filters" onClick={clear}><FilterX size={15} /></button></header>
        <div className="alert-feed-count"><span>Incident queue</span><strong>{filtered.length} matching alerts</strong></div>
        <div className="alert-feed-rows">{filtered.map((alert) => <button key={alert.id} className={`${alert.severity} ${selected?.id === alert.id ? "selected" : ""}`} onClick={() => setSelectedId(alert.id)}><i /><time>{alert.createdAt}</time><div><strong>{alert.title}</strong><span>{alert.category.replaceAll("_"," ")} / Zone {alert.zone}{alert.bandId ? ` / WB-${String(alert.bandId).padStart(3,"0")}` : ""}</span></div><b>{alert.severity}<small>{alert.status}</small></b><ChevronRight size={14} /></button>)}</div>
      </section>
