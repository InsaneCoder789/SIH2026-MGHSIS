...
export function Configuration() {
  const [weights,setWeights]=useState({hr:20,spo2:20,fall:25,immobility:15,sos:20});
  ...
  return <main className="ops-module-page"><OperationsHeader section="Configuration"/><ModuleTitle eyebrow="Event Rules & Thresholds" title="Settings / Configuration" description="Configure explainable risk weights, deployment mode and operational thresholds for demo behavior." status={saved?"Configuration saved":"Unsaved changes"}/>
    <div className="settings-layout"><section className="settings-nav"><button className="active"><SlidersHorizontal size={16}/>Risk engine</button><button><RadioTower size={16}/>Deployment</button><button><Siren size={16}/>Alerts</button><button><Workflow size={16}/>Interventions</button><button><Settings size={16}/>System</button></section><section className="settings-main"><header><div><p className="eyebrow">Human Risk Engine</p><h2>Configurable weighted scoring</h2><p>These are demonstration thresholds for safety estimation, not clinical diagnostic truth.</p></div><b className={total===100?"valid":"invalid"}>{total}% total</b></header><div className="weight-settings">{([['hr','HR anomaly'],['spo2','SpO2 anomaly'],['fall','Fall detected'],['immobility','Immobility'],['sos','SOS trigger']] as const).map(([key,label])=><label key={key}><div><span>{label}</span><strong>{weights[key]}%</strong></div><input type="range" min="0" max="40" value={weights[key]} onChange={event=>update(key,Number(event.target.value))}/><small>{key==='sos'?'Manual SOS enforces a priority floor.':key==='fall'?'Combined with immobility, raises urgency.':'Weighted into the explainable score.'}</small></label>)}</div><section className="threshold-settings"><header><p className="eyebrow">Risk bands</p><h3>Score classification</h3></header>{[['Low','0 - 29','green'],['Moderate','30 - 54','yellow'],['High','55 - 74','orange'],['Critical','75 - 100','red']].map(([label,range,tone])=><div key={label}><i className={tone}/><span>{label}</span><strong>{range}</strong></div>)}</section><section className="deployment-settings"><header><p className="eyebrow">Deployment mode</p><h3>Venue behavior</h3></header><div>{['CONCERT','CRICKET_STADIUM','PILGRIMAGE'].map(item=><button key={item} className={mode===item?'active':''} onClick={()=>{setMode(item);setSaved(false)}}>{item.replace('_',' ')}</button>)}</div></section><footer><button onClick={()=>{setWeights(defaults);setMode('CRICKET_STADIUM');setSaved(false)}}><RotateCcw size={15}/>Restore defaults</button><button className="save" disabled={total!==100} onClick={()=>setSaved(true)}><Save size={15}/>Save configuration</button></footer></section></div>
  </main>;
}
